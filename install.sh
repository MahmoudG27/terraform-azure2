...
params=("2" "wordpress" "wordpressuser" "P@ssw0rd123@" "2" "10.1.0.10" "mahmoud" "P@ssw0rd123@" "/var/www/wordpress" "http" "wordpress" "test.local")
...
if dpkg -s mysql-server-8.0 >/dev/null 2>&1; then
   ...
   sudo /bin/sed -i "s/username_here/$userdb/g" $install_dir/wp-config.php
   sudo /bin/sed -i "s/password_here/$passworduser/g" $install_dir/wp-config.php
   sudo sed -i "s|define( 'DB_HOST', 'localhost' );|define( 'DB_HOST', '$ipDBMS' );|" $install_dir/wp-config.php
fi
